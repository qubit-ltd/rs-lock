...
impl<T> ArcRwLock<T> {
    ...
    /// Creates a lock wrapper from an existing Arc-wrapped read-write lock.
    ///
    /// # Arguments
    ///
    /// * `inner` - Existing shared read-write lock allocation to wrap.
    ///
    /// # Returns
    ///
    ...
    /// Borrows the Arc that owns the wrapped read-write lock.
    ///
    /// # Returns
    ///
