...
fn test_loom_waiter_registry_notify_one_races_with_cancellation() {
    model(|| {
        ...
        let cancellation = thread::spawn(move || cancellation_registry.unregister(cancelled_waiter_id));

        assert_eq!(notifier.join().expect("model notifier should finish"), Some(10));
        assert_eq!(cancellation.join().expect("model cancellation should finish"), Some(20),);
...
fn test_loom_waiter_registry_notify_one_races_with_selected_cancellation() {
    model(|| {
        ...
        let cancelled = cancellation.join().expect("model cancellation should finish");
        assert!(
