...
arc_blocking_monitor_contract_tests!(
    arc_parking_lot_monitor_contract,
    ArcParkingLotMonitor,
    ParkingLotMonitor
);
...
fn test_arc_parking_lot_monitor_preserves_inner_arc_identity() {
    let inner = Arc::new(ParkingLotMonitor::new(1usize));
    let monitor = ArcParkingLotMonitor::from_arc(Arc::clone(&inner));
...
#[test]
fn test_arc_parking_lot_monitor_traits_delegate_to_monitor_methods() {
    let monitor = ArcParkingLotMonitor::new(vec![1, 2]);

    <ArcParkingLotMonitor<Vec<i32>> as Notifier>::notify_one(&monitor);
    ...
    assert_eq!(
        <ArcParkingLotMonitor<Vec<i32>> as ConditionWaiter>::wait_while(
            &monitor,
            |items| items.is_empty(),
            |items| {
                items.push(3);
    ...
    assert_time_result_eq!(
        <ArcParkingLotMonitor<Vec<i32>> as TimeoutConditionWaiter>::wait_until_for(
            &monitor,
            Duration::ZERO,
            |items| !items.is_empty(),
            |items| items.pop().expect("item should be ready"),
