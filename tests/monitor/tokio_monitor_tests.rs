...
async fn test_tokio_monitor_completion_error_wins_over_post_wait_readiness() {
    let monitor = TokioMonitor::with_timer(false, Arc::new(completion_failing_timer()));
...
async fn test_tokio_monitor_post_notification_timer_error_wins_over_readiness() {
    let (timer, timer_polls) = TwicePendingTimer::new(completion_failing_timer());
    ...
    let error = result.expect_err("post-notification Timer failure should be visible");
    assert_backend_unavailable(error);
...
async fn test_tokio_monitor_uses_injected_manual_timer_without_real_delay() {
    ...
    let waiter = tokio::spawn(async move {
        waiter_monitor
            .wait_until_for_async(Duration::from_secs(8), |ready| *ready, |_| ())
            .await
    });
...
fn test_tokio_monitor_uses_timer_across_runtimes() {
    ...
    let mut wait = Box::pin(monitor.wait_until_for_async(Duration::from_secs(5), |ready| *ready, |_| ()));
    ...
    assert_time_result_eq!(Ok(WaitTimeoutResult::TimedOut), polling.block_on(wait),);
}
...
async fn test_tokio_monitor_cancellation_removes_manual_timer_registration() {
    ...
    waiter.abort();
    let error = waiter.await.expect_err("aborted waiter task should be cancelled");
...
async fn test_tokio_monitor_propagates_timer_registration_error() {
    let monitor = TokioMonitor::with_timer(false, Arc::new(registration_failing_timer()));
...
async fn test_tokio_monitor_ready_predicate_skips_timer_registration() {
    let monitor = TokioMonitor::with_timer(true, Arc::new(registration_failing_timer()));

    let result = monitor.wait_until_for_async(Duration::MAX, |ready| *ready, |_| 7).await;
...
fn poll_once<F: Future + ?Sized>(future: Pin<&mut F>, wake_counter: &Arc<WakeCounter>) -> Poll<F::Output> {
...
async fn test_tokio_monitor_notify_one_does_not_lose_registered_waiter() {
    ...
    let first = monitor.wait_until_async(|available| *available > 0, |available| *available -= 1);
    let second = monitor.wait_until_async(|available| *available > 0, |available| *available -= 1);
    tokio::pin!(first);
    tokio::pin!(second);
...
async fn test_tokio_monitor_signal_reacquire_crossing_deadline_times_out() {
    ...
    let holder = thread::spawn(move || {
        ...
        runtime.block_on(holder_monitor.with_write_async(|_| {
            holding_tx.send(()).expect("controller should observe held state lock");
            release_rx
    ...
    release_tx.send(()).expect("holder should receive release permission");
...
fn test_tokio_monitor_notify_all_selects_registered_waiters_only() {
    ...
    assert_eq!(REGISTERED_WAITERS + 1, predicate_checks.load(Ordering::SeqCst));
}
...
fn test_tokio_monitor_cancelled_selected_waiter_discards_notification() {
    let clock = ManualMonotonicClock::new_shared();
    let monitor = TokioMonitor::with_timer((), clock.new_timer());
    let predicate_checks = [Arc::new(AtomicUsize::new(0)), Arc::new(AtomicUsize::new(0))];
    let mut waiters: Vec<Option<Pin<Box<dyn Future<Output = ()> + Send>>>> = predicate_checks
        .iter()
        .map(|checks| {
        ...
        .collect();
    let wake_counters = [Arc::new(WakeCounter::default()), Arc::new(WakeCounter::default())];

    for (waiter, wake_counter) in waiters.iter_mut().zip(&wake_counters) {
        assert!(
            poll_once(
                waiter.as_mut().expect("registered waiter should still exist").as_mut(),
                wake_counter,
            )
            .is_pending()
        );
    ...
    assert_eq!(
        1,
        wake_counters.iter().map(|counter| counter.count()).sum::<usize>(),
...
async fn test_tokio_monitor_deadline_wins_over_simultaneous_notification() {
    ...
    let mut waiter = Box::pin(monitor.wait_until_for_async(Duration::from_secs(1), |ready| *ready, |_| ()));
    let wake_counter = Arc::new(WakeCounter::default());
...
async fn assert_panicking_combined_mutation_does_not_notify(notify_all: bool) {
    ...
    assert!(mutation.await.expect_err("mutation should panic").is_panic());
...
async fn test_tokio_monitor_reports_timeout_duration_overflow() {
    ...
    let mut waiter = Box::pin(monitor.wait_while_for_async(Duration::MAX, |_| true, |_| ()));
    let wake_counter = Arc::new(WakeCounter::default());
...
async fn test_tokio_monitor_condition_wait_futures_are_send() {
    ...
    assert!(
        assert_send(<TokioMonitor<bool> as AsyncConditionWaiter>::wait_until_async(
            &monitor,
            |ready| *ready,
            |ready| *ready,
        ),)
    ...
    assert_send(<TokioMonitor<bool> as AsyncConditionWaiter>::wait_until_ready_async(
        &monitor,
        |ready| *ready,
    ))
    .await;
    assert!(
        assert_send(<TokioMonitor<bool> as AsyncConditionWaiter>::wait_while_async(
            &monitor,
            |ready| !*ready,
            |ready| *ready,
        ),)
...
async fn test_tokio_monitor_helpers_delegate_to_state() {
    ...
    assert_eq!(monitor.with_read_async(|items| items.clone()).await, vec![1, 2],);
    ...
    assert!(empty_monitor.with_read_async(|items| items.is_empty()).await);
}
...
async fn test_tokio_monitor_traits_delegate_to_monitor_methods() {
    ...
    monitor.with_write_notify_one_async(|items| items.push(2)).await;
    assert_eq!(condition_wait.await, 2);

    let timeout_condition_wait = <TokioMonitor<Vec<i32>> as AsyncTimeoutConditionWaiter>::wait_until_for_async(
        &monitor,
        Duration::from_secs(1),
        |items| !items.is_empty(),
        |items| items.pop().expect("item should be ready"),
    ...
    monitor.with_write_notify_one_async(|items| items.push(1)).await;
    assert_time_result_eq!(timeout_condition_wait.await, Ok(WaitTimeoutResult::Ready(1)),);
}
...
async fn test_tokio_monitor_async_wait_while_with_deadline_includes_pre_poll_time() {
    ...
    let wait = monitor.wait_while_with_deadline_async(deadline, |_| true, |_| ());
...
async fn test_tokio_monitor_deadline_helpers_wait_and_complete() {
    ...
    let mut ready_wait = Box::pin(ready_monitor.wait_until_with_deadline_async(ready_deadline, |ready| *ready, |_| 7));
    let wake_counter = Arc::new(WakeCounter::default());

    assert!(poll_once(ready_wait.as_mut(), &wake_counter).is_pending());
    ready_monitor.with_write_notify_one_async(|ready| *ready = true).await;
    ...
    let mut timeout_wait =
        Box::pin(timeout_monitor.wait_until_ready_with_deadline_async(timeout_deadline, |ready| *ready));
    ...
    let repeat_deadline = clock
        ...
        .checked_add(WAIT_TIMEOUT)
        .expect("manual clock deadline should be representable");
    let mut repeat_wait = Box::pin(repeat_monitor.wait_while_with_deadline_async(repeat_deadline, |_| true, |_| ()));
...
async fn test_tokio_monitor_async_wait_while_for_excludes_initial_lock_contention_from_timeout() {
    ...
    let holder = thread::spawn(move || {
        let runtime = tokio::runtime::Builder::new_current_thread()
            ...
            .expect("holder runtime should build");
        runtime.block_on(holder_monitor.with_write_async(|_| {
            holding_tx.send(()).expect("test should observe the held state lock");
            release_rx.recv().expect("holder should receive release permission");
        }));
    });
    holding_rx.recv().expect("holder should acquire the state lock");

    let wait = monitor.wait_while_for_async(Duration::from_millis(5), |ready| !*ready, |_| ());
    tokio::pin!(wait);
    assert!(
        std::future::poll_fn(|context| { Poll::Ready(wait.as_mut().poll(context)) })
            .await
...
async fn test_tokio_monitor_async_wait_while_for_reuses_fixed_timeout_deadline() {
    let monitor = TokioMonitor::current(false);
    let wait = monitor.wait_while_for_async(Duration::from_millis(10), |ready| !*ready, |_| ());
    tokio::pin!(wait);
    assert!(
        std::future::poll_fn(|context| { Poll::Ready(wait.as_mut().poll(context)) })
            .await
