...
fn test_tokio_monitor_uses_timer_across_runtimes() {
    ...
    let mut wait =
        Box::pin(monitor.wait_until_for_async(Duration::from_secs(5), |ready| *ready, |_| ()));
...
async fn test_tokio_monitor_deadline_wins_over_simultaneous_notification() {
    ...
    let mut waiter =
        Box::pin(monitor.wait_until_for_async(Duration::from_secs(1), |ready| *ready, |_| ()));
    let wake_counter = Arc::new(WakeCounter::default());
...
async fn test_tokio_monitor_condition_wait_futures_are_send() {
    ...
    assert_send(
        <TokioMonitor<bool> as AsyncConditionWaiter>::wait_until_ready_async(&monitor, |ready| {
            *ready
        }),
    )
    .await;
